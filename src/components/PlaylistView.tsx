...
export const PlaylistView: React.FC = () => {
  ...
  const validCustomCover =
    playlist?.image &&
    playlist.image !== 'https://i.ytimg.com/vi/D47mUu1b_54/hqdefault.jpg' &&
    !playlist.image.includes('D47mUu1b_54')
      ? playlist.image
      : '';
  ...
  return (
    <div id="playlist-page" className="pb-36 min-h-screen bg-transparent text-white select-none relative">
      ...
      {/* 1. Immersive Wide Backdrop Cover Header matching Top 50 Indonesia & ArtistView Style */}
      <div className="relative w-full h-[370px] sm:h-[430px] overflow-hidden bg-neutral-950">
        {bannerCover ? (
          ...
          <div className="w-full h-full bg-gradient-to-b from-neutral-800 to-neutral-950 flex items-center justify-center">
            <Music2 className="w-24 h-24 text-white/20" />
          </div>
        )}

        {/* Soft Dark Vignette & Bottom Gradient */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#111113] via-[#111113]/60 to-black/40" />
          ...
          <div className="flex items-center gap-2">
            {isUserPlaylist && playlist && (
              <button
                ...
                className="w-10 h-10 rounded-full bg-black/40 hover:bg-red-500/30 text-white/80 hover:text-red-400 backdrop-blur-md border border-white/15 flex items-center justify-center transition-all cursor-pointer shadow-lg active:scale-95"
        ...
        <div className="absolute bottom-4 left-0 right-0 px-5 sm:px-8 z-20">
          ...
          <h1 className="text-3xl sm:text-5xl font-black tracking-tight text-white mb-2 drop-shadow-md line-clamp-2">
            {playlist?.name || 'Playlist'}
          </h1>

          {playlist?.description && (
            <p className="text-xs sm:text-sm text-white/70 max-w-xl mb-4 drop-shadow line-clamp-2">
              {playlist.description}
            </p>
          )}
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Round Play/Pause Button on Right */}
            <button
              onClick={isPlaylistPlaying ? togglePlay : handlePlayAll}
              disabled={songs.length === 0}
              className="w-12 h-12 rounded-full bg-emerald-500 hover:bg-emerald-400 text-black flex items-center justify-center shadow-2xl ml-auto active:scale-95 transition-all cursor-pointer disabled:opacity-50"
              title={isPlaylistPlaying ? 'Jeda' : 'Putar Semua'}
            >
              {isPlaylistPlaying ? (
