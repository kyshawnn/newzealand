    ...
    setPlaylists((prev) =>
      prev.map((pl) => {
        if (pl.id === playlistId) {
          ...
          const updatedImage =
            pl.image &&
            pl.image !== 'https://i.ytimg.com/vi/D47mUu1b_54/hqdefault.jpg' &&
            !pl.image.includes('D47mUu1b_54')
              ? pl.image
              : (songImg || '');

          return { ...pl, songs: updatedSongs, image: updatedImage };
        }
        return pl;
      })
    );
      ...
      prev.map((pl) => {
        if (pl.id === playlistId) {
          const filteredSongs = pl.songs.filter(
            (s) => s.id !== songId && (!s.videoId || s.videoId !== songId)
          );
          const nextFirst = filteredSongs[0];
          const nextCover =
            nextFirst?.image ||
            (nextFirst?.videoId ? `https://i.ytimg.com/vi/${nextFirst.videoId}/hqdefault.jpg` : '');
